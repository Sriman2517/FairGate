...
export async function readWaitingRoom(showId: unknown, join = false): Promise<WaitingRoomResult> {
  ...
  try {
    const response = await bookingRequest(`/waiting-room/${encodeURIComponent(showId)}${join ? "/join" : ""}`, {
    ...
    return { error: "We could not check your turn. Please try again shortly." };
  } catch (error) {
