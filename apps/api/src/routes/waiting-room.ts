...
import { getWaitingRoom, leaveWaitingRoom } from "../waiting-room.js";
import { enforceRequestLimit } from "../request-limits.js";

export const waitingRoomRouter = Router();

// Only POST /join creates membership. POST /leave removes only the signed-in account.
waitingRoomRouter.route("/:showId{/:operation}").all(async (request, response, next) => {
  ...
  const leave = request.method === "POST" && request.params.operation === "leave";
  if (!join && !leave && !(request.method === "GET" && !request.params.operation)) { next(); return; }
  const user = await currentUser(request);
  if (!user) {
    response.status(401).json({ error: { code: "UNAUTHENTICATED", message: "Sign in to join the waiting room." } });
    return;
  }
  // Joins, status checks, and leaves share the same account budget across shows and sessions.
  ...
  if (!leave && show.startsAt <= new Date()) {
  ...
  const waitingRoom = leave
    ? await leaveWaitingRoom(user.id, showId, show.startsAt, available !== null)
    : await getWaitingRoom(user.id, showId, show.startsAt, join);
